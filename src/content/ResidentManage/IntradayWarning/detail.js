...
class IntradayWarning extends Component{
  constructor(props){
    super(props);
    this.state={
      dataSource: [],
  ...
  _date(){
    let adate = new Date();
    let times = adate.getFullYear() + "-" + (adate.getMonth()+1) + "-" + adate.getDate() + " " + adate.getHours() + ":" + adate.getMinutes() + ":" +  adate.getSeconds();
    this.setState({
      startValue: moment(times, "YYYY-MM-DD HH:mm:ss"),
      endValue: moment(times, "YYYY-MM-DD HH:mm:ss"),
    })
  }
  ...
  _onModal_1(type,e){
    ...
    //其他选项理由
    else if(type === "other") this.setState({ otherInput: e.target.value })
    //备注内容
  ...
  render(){
    ...
    return (
      <Row>
        ...
        <Row type="flex" className="intraday-detail-datepicker" gutter={24}>
          <Col lg={6} md={8} xs={12}>
            <DatePicker
...
export default IntradayWarning;