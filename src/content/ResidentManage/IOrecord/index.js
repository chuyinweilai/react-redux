...
import IORecordDetail from './detail'

class IOrecord extends Component{
  constructor(props){
    super(props);
    this.state={
      dataSource: [],
      ...
      <Row>
        <Col>
          <div className="IOrecord-More">
            <Link to='/contant/ResidentManage/IOrecord/detail'><Button type='primary'>点击查看更多</Button></Link>
          </div>
        ...
        <IORecordDetail showDetail={false} per_page={5}/>
        </Col>
        <h3 className="IOrecord-title">热点大门记录</h3>  
        <Col>
          <Row type="flex" span={24} className="IOrecord-Door">
            {this.state.dataSource_2.map(this._topCards.bind(this))}
          </Row>
        </Col>
        <h3 className="IOrecord-title">热点楼栋记录</h3>  
        <Col>
          <Row type="flex" span={24} className="IOrecord-Door">
            {this.state.dataSource_3.map(this._topCards.bind(this))}
          </Row>
        </Col>
				<Modal
